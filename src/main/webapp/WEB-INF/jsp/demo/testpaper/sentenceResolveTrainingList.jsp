                ...
                <div class="modal-content">
                    <div class="wrapper wrapper-content animated fadeInRight" id="divTable1">
                        ...
                        <div class="form-group">
                            ...
                            <%--占七列 并且造一个input组--%>
                            <div class="col-sm-3 input-group">
                                <select id="states3"  class="form-control">
                                    <option value="">请选择辅助时长</option>
                        ...
                        <div class="form-group">
                            ...
                            <%--占七列 并且造一个input组--%>
                            <div class="col-sm-3 input-group">
                                <select id="states4"  class="form-control">
                                    <option value="">请选择辅助时长</option>
...
<script >
    $(document).ready(function () {
        ...
        var obj3 = document.getElementById("states3")
        var obj4 = document.getElementById("states4")
        for (var i = 0; i < 60; i++) {
            obj1.options.add(new Option(i + 1, i + 1)); //这个兼容IE与firefox
            obj2.options.add(new Option(i + 1, i + 1)); //这个兼容IE与firefox
            obj3.options.add(new Option(i + 1, i + 1)); //这个兼容IE与firefox
            obj4.options.add(new Option(i + 1, i + 1)); //这个兼容IE与firefox
        }
        var helptime ='';
        //获取辅助时长
        $.ajax({
...
//                + '<button class="sendEmail btn btn-success" style="margin-left: 10px">发送邮件</button></div>',
            ...
            serverSide: true,
            ajax: {
                "url": "${path}/manage/sentence/getSentenceResolveTrainingList",
                "data": function (d) {
        ...
        $(".bt-add").click(function(){
            window.location.href = "${path}/manage/sentence/toAddSentenceResolveTraining";
        });
        ...
        $dataTable.on('click', ".btn-edit", function () {
            var id = $(this).attr("id");
            window.location.href = "${path}/manage/sentence/toAddSentenceResolveTraining?id=" + id;
        });
        $dataTable.on('click', ".btn-check", function () {
            var id = $(this).attr("id");
            window.location.href = "${path}/manage/sentence/toCheckSentenceResolveTraining?id=" + id;
        });
        //设置时长
        $(".btn-addOK").click(function () {
            var states1 = $("#states1").val()
            var states2 = $("#states2").val()
            var states3 = $("#states3").val()
            var states4 = $("#states4").val()
            $.ajax({
        ...
        $dataTable.on('click',".btn-delete",function () {
            ...
            $.ajax({
                url:"${path}/manage/sentence/deleteSentenceResolveTraining",
                dataType:'JSON',
                type: 'POST',
                data:{
                    id:id,

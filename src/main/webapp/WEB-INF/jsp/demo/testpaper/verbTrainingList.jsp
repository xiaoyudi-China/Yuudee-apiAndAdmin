                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-3 input-group">
                            <select id="states1"  class="form-control">
                                ...
                                <%--<c:forEach items="${countiyList}" var="item"  varStatus="status">
                                    <option value="${ status.index + 1}" <c:if test="${helptime.helpTime==(status.index + 1)} }"> selected="selected"</c:if>>${ status.index + 1}</option>
                                </c:forEach>--%>
                            </select>
                        </div>
                    </div>
...
//                + '<button class="sendEmail btn btn-success" style="margin-left: 10px">发送邮件</button></div>',
            ...
            serverSide: true,
            ajax: {
                "url": "${path}/manage/verb/getVerbTrainingList",
                "data": function (d) {
        ...
        $(".bt-add").click(function(){
            window.location.href = "${path}/manage/verb/toAddVerbTraining";
        });
        ...
        $dataTable.on('click', ".btn-edit", function () {
            var id = $(this).attr("id");
            window.location.href = "${path}/manage/verb/toAddVerbTraining?id=" + id;
        });
        //设置时长
        ...
        $dataTable.on('click',".btn-delete",function () {
            ...
            $.ajax({
                url:"${path}/manage/verb/deleteVerbTraining",
                dataType:'JSON',
                type: 'POST',
                data:{
                    id:id,
    ...
    $('#image_manage_Menu11').addClass('active');

...
<script>
    $(function () {
        ...
        function Default(){
            ...
            getStudyInfo({timeType:'1'},function (data) {
                ...
                data.nonuList.forEach(function (item,i) {
                    X.push('第'+(i+1)+'次')
                    Y.push((item.accuracy*100).toFixed(2))
                });
        ...
        function monthQuery(params){
            ...
            getStudyInfo(params,function (data) {
               ...
               /* arrWeek = getInfo(data.time.split('-')[0],data.time.split('-')[1])*/
                ...
                var Percentage = [];
                var stayTime = [];
                data.nounResult.forEach(function (item,i) {
                    weeks.push('第'+(i+1)+'周')
                    weeks2.push('第'+(i+1)+'周')
                    if (item.accuracyList[0] == undefined || item.accuracyList[0].accuracy==undefined ){
                        Percentage.push(0)
                    }else {
                        Percentage.push((item.accuracyList[0].accuracy*100).toFixed(2))
                    }

                    if (item.timeList[0] == undefined || item.timeList[0].countTime==undefined ){
                        stayTime.push(0)
                    }else {
                        stayTime.push(((item.timeList[0].countTime/60)/60).toFixed(2))
                    }
                })
        ...
        $('.month span').click(function () {
            ...
            function indexS(json) {
                ...
                StudyInfoData[json].forEach(function (item,i) {
                    if (item.accuracyList[0] == undefined || item.accuracyList[0].accuracy==undefined ){
                        ...
                        Y.push((item.accuracyList[0].accuracy*100).toFixed(2));
                    }

                    if (item.timeList[0] == undefined || item.timeList[0].countTime==undefined ){
                        Y2.push(0);
                    }else {
                        Y2.push(((item.timeList[0].countTime/60)/60).toFixed(2));
                    }
                ...
                }else if(json == 'vebResult'){
                    text1 = '动词结构短语本月测试正确率统计'
                    text2 = '动词结构短语本月学习市场统计'
                }else if(json == 'sentenceGroupResult'){
            ...
            if(ind == 0){
                datas = indexS('nounResult')
                datas2 = indexS('nounResult')
            }else if(ind == 1){
                datas = indexS('vebResult')
                datas2 = indexS('vebResult')
            }else if(ind == 2){
                datas = indexS('sentenceGroupResult')
                datas2 = indexS('sentenceGroupResult')
            }else{
                datas = indexS('sentenceResolveResult')
                datas2 = indexS('sentenceResolveResult')
            }
            monthNounOption.yAxis = {
                splitNumber: 6,
                name: '正确率/百分比',
